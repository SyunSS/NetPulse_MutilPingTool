...
def run_probe_process(cmd, idle_timeout, is_reply=None):
    """Run a probe and kill it when no output arrives before the deadline."""
    ...
    try:
        while True:
            ...
            if not chunk:
                ...
                return b"".join(chunks)
            chunks.append(chunk)
            if is_reply is None or is_reply(chunk):
                deadline = time.monotonic() + idle_timeout
    except Exception:
        try:
            proc.kill()
...
def run_ping(host):
    try:
        ...
        out = run_probe_process(
            cmd, idle_timeout=3,
            is_reply=lambda chunk: b"bytes from" in chunk or b"icmp_seq" in chunk
...
def run_tcping(host, port):
    exe = os.path.join(BASE_DIR, "tcping")
    if not os.path.exists(exe):
        return "Timeout", "100%"
    try:
        out = run_probe_process(
            [exe, "--count", str(TcpingCount), "--timeout", "1", host, str(port)],
            idle_timeout=3,
            is_reply=lambda chunk: b"time=" in chunk.lower()
        ).decode(errors="ignore")
