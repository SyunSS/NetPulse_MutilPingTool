...
TCPING_EXE = os.path.join(BASE_DIR, "tcping")
...
def run_ping(host):
    try:
        cmd = ["ping", "-c", str(PingCount), "-W", "1000", host]
        ...
        while True:
            ...
            if not chunk:
                ...
                break
            chunks.append(chunk)
            if "bytes from" in chunk or "icmp_seq" in chunk:
                deadline = time.time() + 3
        out = "".join(chunks)
...
def run_tcping(host, port):
    ...
    try:
        cmd = [
            TCPING_EXE, "--count", str(TcpingCount),
            "--timeout", "1", host, str(port)
        ]
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        chunks = []
        deadline = time.time() + 3
...
def is_success(avg, loss):
    try:
        return avg != "Timeout" and float(loss.rstrip("%")) < 100.0
    except:
        return False
...
def worker(idx, host, port):
    ...
    if mode == "1":
        ...
        if port:
            ...
            avg, loss, stddev = run_ping(host)
            if is_success(avg, loss):
                line = "%s,ICMP,%s,%s,%s" % (host, avg, loss, stddev)
            else:
                avg80, loss80 = run_tcping(host, 80)
                if is_success(avg80, loss80):
                    line = "%s,TCP:80,%s,%s" % (host, avg80, loss80)
                else:
                    avg443, loss443 = run_tcping(host, 443)
                    if is_success(avg443, loss443):
                        line = "%s,TCP:443,%s,%s" % (host, avg443, loss443)
                    else:
                        line = "%s,ALL_FAIL,Timeout,100%%" % host
