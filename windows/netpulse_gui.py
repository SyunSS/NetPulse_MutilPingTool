...
def _terminate_process(proc):
    """Terminate a probe process and its children without waiting on stdout."""
    ...
    try:
        ...
        while not eof:
            ...
            if remaining <= 0:
                ...
                break
            try:
                line = output_q.get(timeout=min(0.05, remaining))
            except queue.Empty:
                continue
            if line is None:
                eof = True
                continue
            lines.append(line)
            if line_callback and line_callback(line):
                deadline = time.monotonic() + idle_timeout
...
def run_ping(host, count, timeout_mult=3):
    try:
        ...
        out = subprocess.check_output(
            cmd, stderr=subprocess.STDOUT, timeout=count * timeout_mult + 1,
            startupinfo=startupinfo
        ...
        if port:
            ...
            if avg != "Timeout" and not _is_all_lost(loss):
                ...
                if stop_event.is_set():
                    return idx, host, port, "CANCELLED", "Timeout", "100%", ip_part
