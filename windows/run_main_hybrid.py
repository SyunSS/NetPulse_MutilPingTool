...
def run_probe_process(cmd, idle_timeout, encoding="utf-8", is_reply=None):
    """Run a probe until it finishes or stops producing useful output."""
    proc = subprocess.Popen(
        cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
        text=True, encoding=encoding, errors="ignore"
    )
    output = queue.Queue()
    ...
    try:
        while True:
            if time.monotonic() >= deadline:
                ...
                break
            try:
                line = output.get(timeout=min(0.05, deadline - time.monotonic()))
            except queue.Empty:
                continue
            if line is None:
                break
            lines.append(line)
            if is_reply and is_reply(line):
                deadline = time.monotonic() + idle_timeout
        return "".join(lines)
    finally:
...
def run_tcping(host, port):
    ...
    cmd = [exe, "-n", str(TcpingCount), "-w", "3000", host, str(port)]
